...
class VideoRecurrentModel(VideoBaseModel):
    ...
    def setup_optimizers(self):
        ...
        if flow_lr_mul == 1:
            ...
            optim_params = [
                {"params": normal_params, "lr": train_opt["optim_g"]["lr"]},  # add normal params first
                    ...
                    if save_img:
                        if self.opt["is_train"]:
                            ...
                            if self.center_frame_only:  # vimeo-90k
                                ...
                                img_path = osp.join(
                                    self.opt["path"]["visualization"], dataset_name, folder, f"{name_}_{self.opt['name']}.png"
                                )
                            else:  # others
                                img_path = osp.join(
                                    self.opt["path"]["visualization"], dataset_name, folder, f"{idx:08d}_{self.opt['name']}.png"
                                )
