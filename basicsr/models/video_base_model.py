...
class VideoBaseModel(SRModel):
                ...
                if self.opt["is_train"]:
                    ...
                    if self.opt["val"]["suffix"]:
                        save_img_path = osp.join(
                            self.opt["path"]["visualization"], dataset_name, folder, f'{img_name}_{self.opt["val"]["suffix"]}.png'
                        )
                    else:
                        save_img_path = osp.join(
                            self.opt["path"]["visualization"], dataset_name, folder, f'{img_name}_{self.opt["name"]}.png'
                        )
    ...
    def _log_validation_metric_values(self, current_iter, dataset_name, tb_logger):
        ...
        # }
        metric_results_avg = {folder: torch.mean(tensor, dim=0).cpu() for (folder, tensor) in self.metric_results.items()}
