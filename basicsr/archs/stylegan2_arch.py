...
class UpFirDnSmooth(nn.Module):
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}(upsample_factor={self.upsample_factor}" f", downsample_factor={self.downsample_factor})"
...
class StyleGAN2Generator(nn.Module):
    ...
    def __init__(
        self, out_size, num_style_feat=512, num_mlp=8, channel_multiplier=2, resample_kernel=(1, 3, 3, 1), lr_mlp=0.01, narrow=1
    ):
...
class ConvLayer(nn.Sequential):
    ...
    def __init__(
        self, in_channels, out_channels, kernel_size, downsample=False, resample_kernel=(1, 3, 3, 1), bias=True, activate=True
    ):
        layers = []
        # downsample
        if downsample:
...
class StyleGAN2Discriminator(nn.Module):
    ...
    def __init__(self, out_size, channel_multiplier=2, resample_kernel=(1, 3, 3, 1), stddev_group=4, narrow=1):
        ...
        self.final_linear = nn.Sequential(
            EqualLinear(channels["4"] * 4 * 4, channels["4"], bias=True, bias_init_val=0, lr_mul=1, activation="fused_lrelu"),
