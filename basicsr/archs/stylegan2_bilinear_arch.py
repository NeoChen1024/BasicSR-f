...
class StyleGAN2GeneratorBilinear(nn.Module):
    ...
    def __init__(
        self, out_size, num_style_feat=512, num_mlp=8, channel_multiplier=2, lr_mlp=0.01, narrow=1, interpolation_mode="bilinear"
    ):
        ...
        for i in range(num_mlp):
            style_mlp_layers.append(
                EqualLinear(num_style_feat, num_style_feat, bias=True, bias_init_val=0, lr_mul=lr_mlp, activation="fused_lrelu")
            )
...
class ConvLayer(nn.Sequential):
    ...
    def __init__(
        self, in_channels, out_channels, kernel_size, downsample=False, bias=True, activate=True, interpolation_mode="bilinear"
    ):
        layers = []
        self.interpolation_mode = interpolation_mode
        # downsample
        ...
        # conv
        layers.append(
            EqualConv2d(in_channels, out_channels, kernel_size, stride=stride, padding=self.padding, bias=bias and not activate)
        )
        # activation
        if activate:
            if bias:
...
class ResBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, interpolation_mode="bilinear"):
        ...
        self.conv2 = ConvLayer(
            in_channels, out_channels, 3, downsample=True, interpolation_mode=interpolation_mode, bias=True, activate=True
        )
        self.skip = ConvLayer(
            in_channels, out_channels, 1, downsample=True, interpolation_mode=interpolation_mode, bias=False, activate=False
        )
