    ...
    for phase, dataset_opt in opt["datasets"].items():
        if phase == "train":
            ...
            train_loader = build_dataloader(
                train_set, dataset_opt, num_gpu=opt["num_gpu"], dist=opt["dist"], sampler=train_sampler, seed=opt["manual_seed"]
            )
        ...
        while train_data is not None:
            ...
            for sub_step in range(accum_steps):
                ...
                model.optimize_parameters(
                    current_iter, zero_grad=(sub_step == 0), step=(sub_step == accum_steps - 1), accum_steps=accum_steps
                )
